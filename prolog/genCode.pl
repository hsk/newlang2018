...
init_bbs(Lbl)  :- nb_setval(lbl,Lbl),nb_setval(cs,[]),nb_setval(bbs,(H1,H1)).
...
code(mov(A,R),R) :-     integer(A),!,add(mov($A,R)).
...
stmt(E) :-              code(E,_).
func(N:A=B,N:A=BBs) :-  genid('.enter',E),init_bbs(E),maplist(stmt,B),get_bbs(BBs).
