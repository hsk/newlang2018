...
getPush(Cs) :- regp2(Rs),include([R]>>(live(A),m(A:R)),Rs,Cs).
...
func(N:_=BBs,(M1,BBsAlives),N:[]=[N1:[enter(-Size,Rs)|Cs]|BBs1]) :-
  ...
  maplist(bb,BBs,BBsAlives,[N1:Cs|BBs1]),retract(c(Size)),regs2(Regs2),
  include([R]>>retract(m(_:R)),Regs2,Rs),retractall(m(_)).
