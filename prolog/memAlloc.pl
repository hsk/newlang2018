...
code(addl(A,B,C),addl(A1,B1,C1))      :- adr(A,A1),adr(B,B1),adr(C,C1).
code(call(A,B),call(A,B1))            :- adrs(B,B1).
code(ret(A),ret(A1))                  :- adr(A,A1).
code(ifeq(A,B,C,D),ifeq(A1,B1,C1,D1)) :- adr(A,A1),adr(B,B1),adrs(C,C1),adrs(D,D1).
...
func((N,Ls),R) :-
  ...
  R=(N,[subq(Size1,'%rsp')|LL]).
memAlloc(Ls,Rs) :- maplist(func,Ls,Rs).
