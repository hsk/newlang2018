...
:- begin(syntax,[syntax/2]).
  ...
  e ::= eint(ti(i),i) | eadd(e,e) | emul(e,e) | eprint(e) | eblock(e*)
      | evar(id,t) | eid(id) | eassign(e,e) | ecall(e,e*) | earray(e,e) | efield(id,id).
...
:- begin(compile,[compile/2]).
  ...
  e(E,R2) :- (E=eid(_);E=earray(_,_);E=efield(_,_)),!,
...
:-compile([
    ...
    eassign(eid(main),efun([],tv,eblock([
      ...
      eassign(efield(b,x),eint(ti(64),3)),
      eassign(efield(b,y),emul(eint(ti(64),3),eint(ti(64),5))),
      eprint(efield(b,x)),
      eprint(efield(b,y)),
  ...
  syntax(v*,Codes),
  emit('c09.ll',Codes),!,
  shell('llc c09.ll -o c09.s'),
  shell('gcc -static c09.s -o c09.exe'),
  shell('./c09.exe').
:-halt.
