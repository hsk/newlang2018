...
:- begin(compile,[compile/2]).
  add(V) :- assert(v(V)).
  cut(tp(T),T).
  arr(eid(Id),rl(tp(T),Id)) :- env:env(Id,T).
  arr(_,_) :- throw(error).
  e(eint(T,I),rn(T,I)).
  ...
  e(eid(Id),R2) :- arr(eid(Id),R1),t(R1,T1),cut(T1,T2),genreg(T2,R2),add(vload(R2,R1)).
  compile(E,Vs) :- syntax(e,E),resetid,dynamic(env:env/2),e(E,_),findall(V,retract(v(V)),Vs).
:- end(compile).
:- begin(emit,[emit/2]).
  pt(R,X) :- t(R,T),!,pt(T,X).
...
:-compile(eblock([
    evar(c,ti(64)),
    eassign(eid(c),eadd(eint(ti(64),10),eint(ti(64),5))),
    eprint(eid(c))
  ]),Codes),
  syntax(v*,Codes),
  emit('l05.ll',Codes),!,
