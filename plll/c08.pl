...
:- begin(syntax,[syntax/2]).
  ...
  t ::= tv | ti(i) | tp(t) | tfun(t*,t) | tarr(t,i).
  e ::= eint(ti(i),i) | eadd(e,e) | emul(e,e) | eprint(e) | eblock(e*)
      | evar(id,t) | eid(id) | eassign(e,e) | ecall(e,e*) | earray(e,e).
...
:-compile([
    ...
    eassign(eid(main),efun([],tv,eblock([
      evar(a,tarr(ti(64),3)),
      eassign(earray(eid(a),eint(ti(64),1)),eint(ti(64),3)),
      eprint(earray(eid(a),eint(ti(64),1))),
      evar(b,tarr(tarr(ti(64),3),2)),
      eassign(earray(earray(eid(b),eint(ti(64),1)),eint(ti(64),2)),eint(ti(64),3)),
      eprint(earray(earray(eid(b),eint(ti(64),1)),eint(ti(64),2))),
      eprint(ecall(eid(add),[eint(ti(64),3),eint(ti(64),4)]))
    ])))
  ],Codes),
  syntax(v*,Codes),
  emit('c08.ll',Codes),!,
  shell('llc c08.ll -o c08.s'),
  shell('gcc -static c08.s -o c08.exe'),
  shell('./c08.exe').
:-halt.
